...
var firstHero = heroes.shift().slice(0, 8);
...
function doubleTheFun(nums) {
  if (parseInt(nums)){
    return parseFloat(nums) + parseFloat(nums)
  } else {
	return nums+nums;
  }
...
/*
    ...
    Otherwise - return "I love outside"s
*/

function outside(tempNum, humNum, cloNum) {
  if (tempNum > 80 && humNum > 40) {
    return "I'm all sweat"
  } else if (tempNum < 40 && cloNum > 60) {
    return "I have icecicles"
  } else if (tempNum > 80 && humNum < 40 && cloNum < 20 ) {
    return "I'm literally in the desert"
  }else if (tempNum > 80 || humNum > 50 || cloNum > 50) {
